...
import {
  ...
  getExtrasForProduct,
  type ExtraConfig,
} from "@/components/extras/ExtrasModal";
import {
  findBestDiscount,
  type DiscountConfig,
...
type ZoneCheckStatus = "idle" | "checking" | "covered" | "uncovered" | "error";
...
type OrgPaymentMethods = {
  cash?: OrgPMConfig;
  transfer?: OrgPMConfig;
  mercadopago?: OrgPMConfig;
  debit?: OrgPMConfig;
  credit?: OrgPMConfig;
};
type OrgModalities = { salon: boolean; delivery: boolean; takeaway: boolean };
type OrgConfig = {
  paymentMethods?: OrgPaymentMethods | null;
  modalities?: OrgModalities | null;
...
const KANBAN_COLUMNS = [
  {
    status: "NUEVO",
    label: "Nuevos",
    wrapClass: "border-blue-200",
    headClass: "bg-blue-50",
    bodyClass: "bg-blue-50/30",
    badgeClass: "bg-blue-100 text-blue-700",
  },
  {
    status: "EN_PREPARACION",
    label: "En preparación",
    wrapClass: "border-amber-200",
...
export default function ComandasPage() {
  ...
  // ── Extras & discounts state ───────────────────────────────────────────────
  const [allExtras, setAllExtras] = useState<ExtraConfig[]>([]);
  const [allDiscounts, setAllDiscounts] = useState<DiscountConfig[]>([]);
  const [selectedExtras, setSelectedExtras] = useState<SelectedExtra[]>([]);
  const [extrasModalProduct, setExtrasModalProduct] = useState<Product | null>(
    null,
  );
  ...
  const [detailEditDeliveryAddress, setDetailEditDeliveryAddress] =
    useState("");
  ...
  // ── Cancel decisions ────────────────────────────────────────────────────────
  const [cancelStockDecision, setCancelStockDecision] = useState<
    "deduct" | "skip"
  >("skip");
  const [cancelCashDecision, setCancelCashDecision] = useState<"add" | "skip">(
    "skip",
  );
  ...
  const [customerSearchStatus, setCustomerSearchStatus] =
    useState<CustomerSearchStatus>("idle");
  ...
  const [zoneCheckStatus, setZoneCheckStatus] =
    useState<ZoneCheckStatus>("idle");
  ...
  useEffect(() => {
    ...
    const valid =
      (orderType === "SALON" && mod.salon !== false) ||
  ...
  useEffect(() => {
    fetch("/api/organizations/me")
      .then((r) => r.json())
      .then((d: OrgConfig) => setOrgConfig(d));
    fetch("/api/repartidores")
      .then((r) => (r.ok ? r.json() : []))
      .then((d) => {
        if (Array.isArray(d)) setRepartidores(d);
      })
      .catch(() => {});
    fetch("/api/extras?active=true")
      .then((r) => r.json())
      .then((d) => {
        if (Array.isArray(d)) setAllExtras(d);
      })
      .catch(() => {});
    fetch("/api/discounts?active=true")
      .then((r) => r.json())
      .then((d: DiscountConfig[]) => {
        if (Array.isArray(d))
          setAllDiscounts(d.map((x) => ({ ...x, value: Number(x.value) })));
      })
      .catch(() => {});
    Promise.all([
      ...
      const comboArr = Array.isArray(combsData)
        ? combsData
        : (combsData?.data ?? []);
      setProducts(
        prodArr.map(
      ...
      setCombos(
        comboArr.map(
          (x: { id: string; name: string; salePrice: string | number }) => ({
            id: x.id,
            name: x.name,
            salePrice: Number(x.salePrice),
          }),
  ...
  const fetchKanban = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        "/api/sales?orderStatus=NUEVO,EN_PREPARACION,LISTO&limit=200",
      );
      ...
      const currentNuevoIds = new Set(
        orders.filter((o) => o.orderStatus === "NUEVO").map((o) => o.id),
      );
      const newArrivals = Array.from(currentNuevoIds).filter(
        (id) => !prevNuevoIds.current.has(id),
      );
      if (newArrivals.length > 0 && prevNuevoIds.current.size > 0) {
        ...
        if (
          ...
          new Notification(
            `🍽️ ${count} pedido${count > 1 ? "s" : ""} nuevo${count > 1 ? "s" : ""}`,
            {
              body: orders
  ...
  async function moveOrder(
    ...
    newStatus: string,
    opts?: {
      rollbackPayments?: boolean;
      cancelStockDecision?: "deduct" | "skip";
      cancelCashDecision?: "add" | "skip";
    },
  ): Promise<boolean> {
    setMovingId(id);
    try {
  ...
  async function confirmCancel() {
    ...
    setCancelError(null);
    // rollbackPayments only when isPaid and NOT adding to caja
    const needsRollback = (o?.isPaid ?? false) && cancelCashDecision !== "add";
    try {
      await moveOrder(cancellingId, "CANCELADO", {
        rollbackPayments: needsRollback,
        cancelStockDecision,
        cancelCashDecision:
          (o?.isPaid ?? false) ? cancelCashDecision : undefined,
      });
      setCancellingStatus("idle");
      setCancellingId(null);
      setCancelStockDecision("skip");
      setCancelCashDecision("skip");
    } catch (e) {
  ...
  function detailSetQty(id: string, type: DetailEditItem["type"], qty: number) {
    if (qty <= 0) {
      ...
      setDetailEditItems((prev) =>
        prev.map((i) => (i.id === id && i.type === type ? { ...i, qty } : i)),
      );
    }
  }
  ...
  function detailTogglePayment(method: string) {
    ...
    setDetailEditPayments((prev) => {
      ...
      return [
        ...
        {
          method,
          amount: String(Math.max(0, Math.round(currentTotal - sumExisting))),
        },
      ];
    });
  }
  ...
  function getDetailSubtotal() {
    return (
      ...
      (detailOrder?.orderType === "DELIVERY"
        ? Number(orgConfig?.deliveryFee) || 0
        : 0)
    );
  }

  function getDetailTotal() {
    const discount = detailOrder ? Number(detailOrder.discountAmount || 0) : 0;
    return getDetailSubtotal() - discount;
  }
  ...
  async function saveDetailEdits() {
    ...
    await fetch(`/api/sales/${detailId}`, {
      ...
      body: JSON.stringify({
        ...
        newItems: detailEditItems
          .filter((i) => i.type === "product")
          .map((i) => ({ productId: i.id, quantity: i.qty })),
        newComboItems: detailEditItems
          .filter((i) => i.type === "combo")
          .map((i) => ({ comboId: i.id, quantity: i.qty })),
      }),
    });
  ...
  function addOrIncrement(item: {
    ...
    if (item.type === "product") {
      const applicable = getExtrasForProduct(
      ...
      if (applicable.length > 0) {
        ...
        return; // modal will call addProductWithExtras
      }
    }
    setOrder((prev) => {
      const existing = prev.find(
  ...
  function addProductWithExtras(
    product: Product,
    extras: { extraId: string; quantity: number }[],
  ) {
    setOrder((prev) => {
      const existing = prev.find(
        (o) => o.id === product.id && o.type === "product",
      );
      if (existing)
        return prev.map((o) =>
          o.id === product.id && o.type === "product"
            ? { ...o, quantity: o.quantity + 1 }
            : o,
        );
      return [
    ...
    if (extras.length > 0) {
      setSelectedExtras((prev) => {
        ...
        for (const sel of extras) {
          const extraConfig = allExtras.find((e) => e.id === sel.extraId);
          ...
          if (existing) {
            ...
            next.push({
              ...
              price: extraConfig.price,
              isFree: extraConfig.isFree,
            });
          }
        }
        return next;
  ...
  function setQty(id: string, type: OrderItem["type"], qty: number) {
    if (qty <= 0) {
      ...
      setOrder((prev) =>
        prev.map((o) =>
          o.id === id && o.type === type ? { ...o, quantity: qty } : o,
        ),
      );
    }
  }
  ...
  const orgDeliveryFee =
    orderType === "DELIVERY"
      ? matchedZone !== null
        ...
        : Number(orgConfig?.deliveryFee) || 0
      : 0;
  const itemsSubtotal = order.reduce((sum, o) => sum + o.price * o.quantity, 0);
  const extrasAmount = selectedExtras.reduce(
    (sum, e) => sum + (e.isFree ? 0 : e.price * e.quantity),
    0,
  );
  const total = itemsSubtotal + orgDeliveryFee + extrasAmount;
  ...
  const bestDiscount = useMemo(() => {
    ...
    const ctx: DiscountContext = {
      now: new Date(),
      paymentMethod,
      productIds: order.filter((o) => o.type === "product").map((o) => o.id),
      categoryIds: order
        .filter((o) => o.type === "product")
        .map((o) => {
          return products.find((p) => p.id === o.id)?.categoryId ?? "";
        })
        .filter(Boolean),
    };
    return findBestDiscount(allDiscounts, total, ctx);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [allDiscounts, payments, order, total]);
  ...
  const activePaymentMethods = (() => {
    ...
    if (!pm)
      return PM_MAP.map((m) => ({ ...m, cfg: null as OrgPMConfig | null }));
    return PM_MAP.map((m) => ({
      ...m,
      cfg: (pm[m.key] ?? null) as OrgPMConfig | null,
  ...
  const effectiveTotal = (() => {
    if (payments.length !== 1) return totalAfterDiscount;
    const sel = activePaymentMethods.find(
      (m) => m.value === payments[0].method,
    );
    if (!sel?.cfg || sel.cfg.adjustmentType === "none")
      return totalAfterDiscount;
    const factor =
      sel.cfg.adjustmentType === "surcharge"
        ? 1 + sel.cfg.adjustmentPct / 100
        : 1 - sel.cfg.adjustmentPct / 100;
    return Math.round(totalAfterDiscount * factor);
  })();

  const diff = Math.round((effectiveTotal - sumPayments) * 100) / 100;
  ...
  function togglePaymentMethod(method: string) {
    // Compute what the discount would be WITH this method selected, so the
    // default amount reflects the post-discount total even on the first click
    // (before the bestDiscount useMemo has had a chance to re-run).
    const pIds = order.filter((o) => o.type === "product").map((o) => o.id);
    const cIds = order
      .filter((o) => o.type === "product")
      .map((o) => products.find((p) => p.id === o.id)?.categoryId ?? "")
      .filter(Boolean) as string[];
    const methodDiscountAmount =
      findBestDiscount(allDiscounts, total, {
        now: new Date(),
        paymentMethod: method,
        productIds: pIds,
        categoryIds: cIds,
      })?.amount ?? discountAmount; // fall back to current discountAmount if no change
    const baseTotal = total - methodDiscountAmount;
    ...
    setPayments((prev) => {
      ...
      if (existing) {
        ...
        if (remaining.length > 0) {
          const sumOthers = remaining
            .slice(0, -1)
            .reduce((s, p) => s + (Number(p.amount) || 0), 0);
          const last = remaining[remaining.length - 1];
          remaining[remaining.length - 1] = {
            ...last,
            amount: String(Math.max(0, Math.round(baseTotal - sumOthers))),
          };
        }
        return remaining;
      } else {
        const adjTotal =
          prev.length === 0 ? getAdjustedTotal(method, baseTotal) : baseTotal;
        const sumExisting = prev.reduce(
        ...
        return [
          ...
          {
            method,
            amount: String(Math.max(0, Math.round(adjTotal - sumExisting))),
          },
        ];
      }
    });
  ...
  const sumCobrar = cobrarPayments.reduce(
    (s, p) => s + (Number(p.amount) || 0),
    0,
  );

  // Re-evaluate promo discount in cobrar modal when no baked-in discount exists
  const cobrarComputedDiscount = useMemo(() => {
    if (
      !cobrarOrder ||
      (cobrarOrder.discountAmount && Number(cobrarOrder.discountAmount) > 0)
    )
      return null;
    if (allDiscounts.length === 0 || cobrarTotal <= 0) return null;
    const paymentMethod =
      cobrarPayments.length === 1 ? cobrarPayments[0].method : undefined;
    const pIds = cobrarOrder.items.map((i) => i.productId);
    const cIds = pIds
      .map((pid) => products.find((p) => p.id === pid)?.categoryId ?? "")
      .filter(Boolean) as string[];
    const ctx: DiscountContext = {
      now: new Date(),
      paymentMethod,
    ...
    return findBestDiscount(allDiscounts, cobrarTotal, ctx);
  }, [cobrarOrder, cobrarPayments, allDiscounts, cobrarTotal, products]);

  const cobrarEffectiveTotal = (() => {
    const baseAfterDiscount =
      cobrarTotal - (cobrarComputedDiscount?.amount ?? 0);
    if (cobrarPayments.length !== 1) return Math.round(baseAfterDiscount);
    const sel = activePaymentMethods.find(
      (m) => m.value === cobrarPayments[0].method,
    );
    if (!sel?.cfg || sel.cfg.adjustmentType === "none")
      return Math.round(baseAfterDiscount);
    const factor =
      sel.cfg.adjustmentType === "surcharge"
        ? 1 + sel.cfg.adjustmentPct / 100
        : 1 - sel.cfg.adjustmentPct / 100;
    return Math.round(baseAfterDiscount * factor);
  })();
      ...
      if (existing) {
        ...
        if (remaining.length > 0) {
          ...
          remaining[remaining.length - 1] = {
            ...last,
            amount: String(
              Math.max(0, Math.round(baseAfterDiscount - sumOthers)),
            ),
          };
        }
        return remaining;
        ...
        const adjTotal =
          prev.length === 0
            ? getAdjustedTotal(method, baseAfterDiscount)
            : baseAfterDiscount;
        const sumExisting = prev.reduce(
          (s, p) => s + (Number(p.amount) || 0),
          0,
  ...
  async function confirmCobrar() {
    ...
    // Compute adjustment for cobrar modal
    const cobrarAdjMethod =
      savedPayments.length === 1
        ? activePaymentMethods.find((m) => m.value === savedPayments[0].method)
        : null;
    const cobrarHasAdj =
      cobrarAdjMethod?.cfg && cobrarAdjMethod.cfg.adjustmentType !== "none";
    const savedCobrarComputedDiscount = cobrarComputedDiscount;
    const res = await fetch(`/api/sales/${cobrandoId}/pay`, {
      ...
      body: JSON.stringify({
        ...
        ...(cobrarIsCobrado ? { total: effectiveTotalNow } : {}),
        ...(cobrarHasAdj
          ? {
              paymentAdjustmentType: cobrarAdjMethod!.cfg!.adjustmentType,
              paymentAdjustmentPct: cobrarAdjMethod!.cfg!.adjustmentPct,
              paymentAdjustmentAmount: effectiveTotalNow - rawTotalNow,
              paymentMethodSnapshot: orgConfig?.paymentMethods ?? null,
            }
          : {}),
        ...(savedCobrarComputedDiscount
          ? {
              discountAmount: savedCobrarComputedDiscount.amount,
              discountsSnapshot: {
                discountId: savedCobrarComputedDiscount.discount.id,
                name: savedCobrarComputedDiscount.discount.name,
                label: savedCobrarComputedDiscount.discount.label ?? null,
                discountType: savedCobrarComputedDiscount.discount.discountType,
                value: savedCobrarComputedDiscount.discount.value,
                amountApplied: savedCobrarComputedDiscount.amount,
              },
            }
          : {}),
      }),
    ...
    if (res.ok) {
      ...
      if (savedOrder) {
        printReceipt({
          ...
          customerName:
            savedOrder.customer?.name ?? savedOrder.customerName ?? null,
          items: [
  ...
  // ── Delivery zone check ────────────────────────────────────────────────────
  async function checkDeliveryZone(addr: string) {
    if (!addr.trim()) {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        ...
        setZoneCheckStatus("covered");
        setMatchedZone({
          name: data.zone.name,
          price: Number(data.zone.price),
        });
      } else {
        setZoneCheckStatus("uncovered");
        setMatchedZone(null);
      }
    } catch {
      setZoneCheckStatus("error");
  ...
  // Reset zone check when switching away from DELIVERY
  useEffect(() => {
  ...
  // Delivery address autocomplete (server-side proxy)
  const [deliverySuggestions, setDeliverySuggestions] = useState<
    { description: string; placeId: string }[]
  >([]);
  const [showDeliverySuggestions, setShowDeliverySuggestions] = useState(false);
  const deliverySuggestTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );

  async function fetchDeliverySuggestions(input: string) {
    ...
    try {
      ...
      const data = await res.json();
      setDeliverySuggestions(data.predictions ?? []);
      setShowDeliverySuggestions((data.predictions ?? []).length > 0);
    } catch {
      setDeliverySuggestions([]);
    }
  }
  ...
  async function handleSubmit(opts: { withPayment: boolean }) {
    ...
    if (opts.withPayment && paymentsToSend.length === 0) {
      setStatus("error");
      setMessage("Seleccioná al menos un método de pago");
      return;
    }
    ...
    if (
      ...
      Math.abs(
        effectiveTotal -
          paymentsToSend.reduce((s, p) => s + (Number(p.amount) || 0), 0),
      ) > 0.01
    ) {
      const d =
        effectiveTotal -
      ...
      setMessage(
        d > 0
          ? `Faltan ${fmt(d)} para completar el pago`
          : `El pago excede el total por ${fmt(-d)}`,
      );
      return;
    }
    setStatus("loading");
    // Compute adjustment snapshot when exactly 1 method with adjustment
    const adjMethod =
      paymentsToSend.length === 1
        ? activePaymentMethods.find((m) => m.value === paymentsToSend[0].method)
        : null;
    const hasAdj = adjMethod?.cfg && adjMethod.cfg.adjustmentType !== "none";
    const payAdj = effectiveTotal - totalAfterDiscount;
    const discountSnapshot = bestDiscount
      ? {
          discountId: bestDiscount.discount.id,
          name: bestDiscount.discount.name,
          label: bestDiscount.discount.label ?? null,
          discountType: bestDiscount.discount.discountType,
          value: bestDiscount.discount.value,
          amountApplied: bestDiscount.amount,
        }
      : null;
    const body = {
      items: order
        .filter((o) => o.type === "product")
        .map((o) => ({ productId: o.id, quantity: o.quantity })),
      comboItems: order
        .filter((o) => o.type === "combo")
        .map((o) => ({ comboId: o.id, quantity: o.quantity })),
      ...
      deliveryAddress:
        orderType === "DELIVERY" && deliveryAddress.trim()
          ...
          : null,
      repartidorId:
        orderType === "DELIVERY" && repartidorId ? repartidorId : null,
      customerId: selectedCustomer?.id ?? null,
      customerName:
        !selectedCustomer && customerName.trim() ? customerName.trim() : null,
      selectedExtras: selectedExtras.map((e) => ({
        extraId: e.extraId,
        quantity: e.quantity,
      })),
      ...(discountSnapshot
        ? { appliedDiscount: discountSnapshot, discountAmount }
        : {}),
      ...(extrasAmount > 0 ? { extrasAmount } : {}),
      ...(hasAdj
        ? {
            paymentAdjustmentType: adjMethod!.cfg!.adjustmentType,
            paymentAdjustmentPct: adjMethod!.cfg!.adjustmentPct,
            paymentAdjustmentAmount: payAdj,
            paymentMethodSnapshot: orgConfig?.paymentMethods ?? null,
          }
        : {}),
    };
    ...
    if (res.ok) {
      ...
      setMessage(
        withCobrado
          ...
          : opts.withPayment
            ? "¡Pedido guardado con método de pago!"
            : "¡Pedido comandado!",
      );
  ...
  // ── Discount badge per payment method ────────────────────────────────────────
  const discountPerMethod: Record<string, string> = {};
  if (allDiscounts.length > 0 && total > 0) {
    const pIds = order.filter((o) => o.type === "product").map((o) => o.id);
    const cIds = order
      .filter((o) => o.type === "product")
      .map((o) => products.find((p) => p.id === o.id)?.categoryId ?? "")
      .filter((id): id is string => Boolean(id));
    for (const m of PM_MAP) {
      const ctx: DiscountContext = {
        now: new Date(),
      ...
      const best = findBestDiscount(allDiscounts, total, ctx);
      if (best)
        discountPerMethod[m.value] =
          best.discount.discountType === "PERCENTAGE"
            ? `−${best.discount.value}%`
            : `−$${best.discount.value}`;
    }
  }

  // Discounts that apply by scope regardless of payment method (for hint banner)
  const scopeDiscounts = allDiscounts.filter((d) => {
    if (!d.isActive) return false;
    if (d.appliesTo === "ORDER") return true;
    if (d.appliesTo === "PRODUCTS")
      return order.some(
        (o) => o.type === "product" && (d.productIds?.includes(o.id) ?? false),
      );
    if (d.appliesTo === "CATEGORIES") {
      const cIds = order
        ...
        .map((o) => products.find((p) => p.id === o.id)?.categoryId ?? "")
        .filter(Boolean);
      return cIds.some((id) => d.categoryIds?.includes(id) ?? false);
    }
    return false;
  });
      ...
      {status === "success" ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-4 p-6 text-center">
          ...
          <div className="flex flex-col gap-2 w-full">
            <button
              onClick={resetOrder}
              className="w-full py-2.5 text-white rounded-xl font-semibold text-sm transition-colors hover:opacity-90"
            ...
            {saleId && (
              <Link
                href={`/sales/${saleId}`}
                className="w-full py-2.5 border border-gray-200 text-gray-700 rounded-xl font-medium text-sm hover:bg-gray-50 transition-colors text-center block"
              >
                Ver venta
              </Link>
            )}
        ...
        <>
          ...
          {/* Delivery address + zone check + repartidor */}
          {orderType === "DELIVERY" && (
            <div className="px-4 pb-2 shrink-0 space-y-1.5">
              <div className="relative">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    deliverySuggestTimerRef.current = setTimeout(
                      () => fetchDeliverySuggestions(val),
                      300,
                    );
                  }}
                  onBlur={() =>
                    setTimeout(() => setShowDeliverySuggestions(false), 150)
                  }
                  className={`w-full border rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:ring-2 bg-amber-50/60 placeholder-amber-400 ${
                    zoneCheckStatus === "uncovered"
                      ? "border-rose-400 focus:ring-rose-400"
                      : zoneCheckStatus === "covered"
                ...
                {showDeliverySuggestions && deliverySuggestions.length > 0 && (
                  <div className="absolute left-0 right-0 top-full mt-0.5 z-50 bg-white rounded-xl border border-gray-200 shadow-xl overflow-hidden">
                    {deliverySuggestions.map((s) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setDeliverySuggestions([]);
                          setShowDeliverySuggestions(false);
                          checkDeliveryZone(s.description);
                        }}
                        className="w-full text-left px-3 py-2 text-xs text-gray-800 hover:bg-gray-50 border-b border-gray-100 last:border-0 flex items-start gap-2"
                      >
                        <svg
              ...
              {zoneCheckStatus === "covered" && matchedZone && (
                <div className="flex items-center gap-1.5 text-xs text-emerald-700 bg-emerald-50 rounded-lg px-2.5 py-1.5">
                  ...
                  <span className="text-emerald-500">
                    · ${matchedZone.price.toLocaleString("es-AR")}
                  </span>
                </div>
              )}
              {zoneCheckStatus === "uncovered" && (
                <div className="flex items-center gap-1.5 text-xs text-rose-600 bg-rose-50 rounded-lg px-2.5 py-1.5">
                  ...
                  No tenemos cobertura hacia esa dirección
                </div>
              )}
                    ...
                    {!showCreateCustomer ? (
                      ...
                      <div className="flex gap-1.5">
                        <input
                          ...
                          onKeyDown={(e) =>
                            e.key === "Enter" && createAndSelectCustomer()
                          }
                          className="flex-1 border border-emerald-300 rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white"
                          autoFocus
                        />
                        <button
              ...
              order.map((item) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      onClick={() =>
                        setQty(item.id, item.type, item.quantity - 1)
                      }
                      className="w-7 h-7 rounded-lg border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-50 text-sm font-bold"
                    >
                      −
                    </button>
                  ...
                  <button
                    onClick={() => setQty(item.id, item.type, 0)}
                    className="text-gray-300 hover:text-rose-500 transition-colors shrink-0"
                  >
                    <svg
                      className="h-4 w-4"
          ...
          <div className="border-t border-gray-100 p-4 space-y-3 shrink-0">
            {/* Available discount hint (when discount exists but isn't applied yet) */}
            {order.length > 0 &&
              discountAmount === 0 &&
              scopeDiscounts.length > 0 && (
                <div className="flex items-center gap-1.5 rounded-lg bg-emerald-50 border border-emerald-100 px-2.5 py-1.5 text-xs text-emerald-700">
                  <svg
                  ...
                  <span>
                    <strong>
                      {scopeDiscounts[0].label ?? scopeDiscounts[0].name}
                    </strong>
                    {scopeDiscounts[0].paymentMethods?.length
                      ? ` · con ${scopeDiscounts[0].paymentMethods.join(", ")}`
                      : ""}
                  </span>
                </div>
              )}
            {/* Pricing breakdown */}
            {orgDeliveryFee > 0 || extrasAmount > 0 || discountAmount > 0 ? (
              <div className="space-y-1.5">
                ...
                {discountAmount > 0 && bestDiscount && (
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-emerald-700">
                      {bestDiscount.discount.label ??
                        bestDiscount.discount.name}
                    </span>
              ...
              <div className="space-y-0.5">
                {selectedExtras.map((se) => (
                  ...
                  >
                    ...
                    <span>
                      {se.isFree ? "Sin cargo" : fmt(se.price * se.quantity)}
                    </span>
                    <button
                      onClick={() =>
                        setSelectedExtras((prev) =>
                          prev.filter((x) => x.extraId !== se.extraId),
                        )
                      }
            ...
            {payments.length === 1 &&
              ...
              (() => {
                ...
                const diff2 = effectiveTotal - totalAfterDiscount;
                return (
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-gray-500">
                      {sel.cfg.adjustmentType === "discount"
                        ? `Descuento ${sel.cfg.adjustmentPct}%`
                        : `Recargo ${sel.cfg.adjustmentPct}%`}
                    </span>
                    <span
                      className={`font-semibold ${diff2 < 0 ? "text-green-600" : "text-red-500"}`}
                    >
                      {diff2 < 0 ? `−${fmt(-diff2)}` : `+${fmt(diff2)}`} ={" "}
            ...
            <div
              className={`grid gap-1.5 ${activePaymentMethods.length <= 3 ? "grid-cols-3" : activePaymentMethods.length === 4 ? "grid-cols-2 sm:grid-cols-4" : "grid-cols-3"}`}
            >
              {activePaymentMethods.map((m) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[10px] font-semibold mt-0.5 leading-tight text-center">
                      {m.label}
                    </span>
                    {m.cfg?.adjustmentType !== "none" && (
                      <span
                        className={`text-[9px] font-bold ${m.cfg?.adjustmentType === "discount" ? "text-green-500" : "text-red-400"}`}
                      >
                        {m.cfg?.adjustmentType === "discount" ? "−" : "+"}
                        {m.cfg?.adjustmentPct}%
                      </span>
                    )}
                    {!selected && discountPerMethod[m.value] && (
                      <span className="text-[9px] font-bold text-emerald-500 bg-emerald-50 rounded px-1">
                        {discountPerMethod[m.value]}
                      </span>
                    )}
                    {selected && (
                      <input
                        ...
                        onChange={(e) => {
                          e.stopPropagation();
                          updatePaymentAmount(m.value, e.target.value);
                        }}
              ...
              (() => {
                ...
                return (
                  <div className="bg-blue-50 rounded-lg px-3 py-2 text-xs text-blue-700 space-y-0.5">
                    {cfg.bank && (
                      <p>
                        🏦 <span className="font-medium">{cfg.bank}</span>
                      </p>
                    )}
                    {cfg.alias && (
                      <p>
            ...
            {/* MP link */}
            {payments.find((p) => p.method === "ONLINE") &&
              orgConfig?.paymentMethods?.mercadopago?.link && (
                <a
                  href={orgConfig.paymentMethods.mercadopago.link}
                  target="_blank"
            ...
            {payments.length > 0 && (
              <button
                onClick={() => setIsCobrado(!isCobrado)}
              ...
              >
                <span
                  className={`text-xs font-semibold ${isCobrado ? "text-emerald-700" : "text-gray-500"}`}
                >
                  {isCobrado ? "Cobrado" : "Pendiente de cobro"}
                </span>
                <div
                  className={`w-8 h-5 rounded-full flex items-center px-0.5 transition-colors ${isCobrado ? "bg-emerald-500" : "bg-gray-300"}`}
                >
                  <div
                    className={`w-4 h-4 rounded-full bg-white shadow transition-transform ${isCobrado ? "translate-x-3" : "translate-x-0"}`}
                  />
                </div>
              </button>
            )}
            {status === "error" && (
              <p className="text-xs text-rose-600 text-center bg-rose-50 rounded-lg px-3 py-2">
                {message}
              </p>
            )}
            {/* Delivery zone gate */}
            {orderType === "DELIVERY" && zoneCheckStatus === "uncovered" && (
              <p className="text-xs text-rose-600 bg-rose-50 rounded-lg px-3 py-2 text-center font-medium">
                Sin cobertura — corregí la dirección para continuar
              </p>
            )}
            <div className="flex gap-2">
              {(() => {
                ...
                return (
                  <>
                    <button
                      onClick={() => handleSubmit({ withPayment: false })}
                      disabled={
                        order.length === 0 ||
                        status === "loading" ||
                        deliveryBlocked
                    ...
                    <button
                      onClick={() => handleSubmit({ withPayment: true })}
                      disabled={
                        order.length === 0 ||
                        status === "loading" ||
                        payments.length === 0 ||
                        (isCobrado && Math.abs(diff) > 0.01) ||
                        deliveryBlocked
                      }
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="flex gap-3 h-full overflow-x-auto p-3 snap-x snap-mandatory md:snap-none">
        {KANBAN_COLUMNS.map((col) => {
          const colOrders = kanbanOrders.filter(
          ...
          return (
            <Droppable key={col.status} droppableId={col.status}>
              {(provided, snapshot) => (
                ...
                >
                  <div
                    className={`px-3 py-2.5 border-b-2 ${col.wrapClass} ${col.headClass} rounded-t-xl shrink-0`}
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-xs font-bold px-2 py-0.5 rounded-full ${col.badgeClass}`}
                      >
                        {colOrders.length}
                      </span>
                    </div>
                  ...
                  <div
                    className={`flex-1 overflow-y-auto p-2 space-y-2 ${col.bodyClass} rounded-b-xl`}
                  >
                        ...
                        <Draggable key={ko.id} draggableId={ko.id} index={idx}>
                          {(prov, snap) => (
                            <div
                              ...
                              onClick={() => {
                                if (!snap.isDragging) openDetail(ko);
                              }}
                            ...
                            >
                              ...
                              <div className="flex items-start justify-between gap-1">
                                <div className="min-w-0 flex-1">
                                  ...
                                  <div className="flex items-center gap-1.5 mt-0.5">
                                    <p className="text-[11px] text-gray-400">
                                      {relativeTime(ko.date)}
                                    </p>
                                    {ko.delayMinutes != null &&
                                      ko.delayMinutes > 0 && (
                                ...
                                <div className="flex flex-col items-end gap-1 shrink-0">
                                  ...
                                  {!ko.isPaid &&
                                    (() => {
                                      const allPending =
                                        ko.payments.length > 0 &&
                                        ko.payments.every(
                                          (p) => p.confirmedAt === null,
                                        );
                                      if (allPending)
                                        return (
                                          <span className="text-[9px] bg-blue-100 text-blue-700 rounded px-1.5 py-0.5 font-medium">
                                            Pago pendiente
                                          </span>
                                        );
                                      const paid = ko.payments
                                      ...
                                      return paid > 0 ? (
                                        <span className="text-[9px] bg-amber-100 text-amber-700 rounded px-1.5 py-0.5 font-medium">
                                          Falta {fmt(owed)}
                                        </span>
                                      ) : (
                                        <span className="text-[9px] bg-rose-100 text-rose-600 rounded px-1.5 py-0.5 font-medium">
                                          Sin cobrar
                                        </span>
                                      );
                                    })()}
                              ...
                              <p className="text-xs text-gray-600 line-clamp-2">
                                {[
                                  ...ko.items.map(
                                    (i) =>
                                      `${Number(i.quantity)}× ${i.product.name}`,
                                  ),
                                  ...ko.combos.map(
                                    (c) =>
                                      `${Number(c.quantity)}× ${c.combo.name}`,
                                  ),
                                ].join(", ")}
                              </p>

                              {/* Delivery address + repartidor */}
                              {ko.orderType === "DELIVERY" &&
                                (ko.deliveryAddress || ko.repartidor) && (
                                  <div className="space-y-0.5">
                                    {ko.deliveryAddress && (
                              ...
                              <div className="flex gap-1.5">
                                ...
                                <button
                                  onClick={(e) => {
                                    ...
                                    if (ko.isPaid) {
                                      ...
                                      setCobrandoId(ko.id);
                                      setCobrarPayments([]);
                                      setCobrarIsCobrado(true);
                                    }
                                  }}
                                  disabled={unpayingId === ko.id}
                                  title={
                                    ko.isPaid
                                      ? "Cobrado — click para revertir"
                                      : "Cobrar"
                                  }
                                ...
                                >
                                  <span
                                    className={`w-3.5 h-3.5 rounded border-2 flex items-center justify-center shrink-0 ${ko.isPaid ? "bg-emerald-500 border-emerald-500" : "border-gray-400"}`}
                                  >
                                    {ko.isPaid && (
                                      <svg
  ...
  return (
    <div
      className={`h-[calc(100vh-3.5rem)] lg:h-screen flex flex-col overflow-hidden -m-4 md:-m-6 lg:-m-8 ${
    ...
    >
      ...
      <div className="flex flex-col min-h-0 bg-gray-50 flex-1">
        {/* Header */}
        <div
          className="bg-white border-b border-gray-200 px-4 py-3 shrink-0 space-y-2"
          style={{ borderTop: "2px solid #0f2f26" }}
        >
          <div className="flex items-center justify-between gap-2">
            <div className="flex gap-1 p-0.5 bg-gray-100 rounded-lg">
              <button
                onClick={() => setPageTab("nueva")}
                className="px-3 py-1.5 rounded-md text-xs font-semibold transition-colors"
                style={
                  pageTab === "nueva"
                    ? { background: "#0f2f26", color: "#fff" }
                    : { color: "#6b7280" }
                }
              >
                Nueva
            ...
            <>
              ...
              <div className="flex flex-wrap gap-1">
                ...
                {categories.map((cat) => (
                  <button
                    key={cat.id}
                    onClick={() => setTab(tab === cat.id ? "all" : cat.id)}
                    className="px-3 py-1 rounded-full text-xs font-semibold transition-colors"
                    style={
                      tab === cat.id
        ...
        {pageTab === "nueva" ? (
          <>
            <div className="flex-1 overflow-y-auto p-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                {filteredProducts.map((prod) => {
                  const inOrder = order.find(
                    (o) => o.id === prod.id && o.type === "product",
                  );
                  return (
                    <button
                      key={prod.id}
                      ...
                      style={
                        inOrder
                          ? { borderColor: "#0f2f26", background: "#f0f7f4" }
                          : { borderColor: "#f3f4f6", background: "#fff" }
                      }
                    >
                      {inOrder && (
                        <span className="absolute top-1.5 right-1.5 min-w-[20px] h-5 flex items-center justify-center rounded-full bg-emerald-600 text-white text-[10px] font-bold px-1">
                          {inOrder.quantity}
                        </span>
                      )}
                      <p className="text-sm font-semibold text-gray-800 leading-tight line-clamp-2 pr-5">
                        {prod.name}
                      </p>
                      <p
                ...
                {filteredCombos.map((combo) => {
                  ...
                  return (
                    ...
                    >
                      {inOrder && (
                        <span className="absolute top-1.5 right-1.5 min-w-[20px] h-5 flex items-center justify-center rounded-full bg-violet-600 text-white text-[10px] font-bold px-1">
                          {inOrder.quantity}
                        </span>
                      )}
                      <p className="text-sm font-semibold text-gray-800 leading-tight line-clamp-2 pr-5">
                      ...
                      <span className="absolute bottom-1.5 right-1.5 text-[9px] text-violet-400 font-medium">
                        COMBO
                      </span>
                    </button>
                  );
            ...
            {order.length > 0 && !showOrder && (
              <div className="lg:hidden shrink-0 border-t border-gray-200 bg-white px-4 py-3 flex items-center justify-between">
                ...
                <button
                  onClick={() => setShowOrder(true)}
                  className="px-5 py-2.5 text-white font-semibold rounded-xl text-sm transition-colors hover:opacity-90"
                  style={{ background: "#0f2f26" }}
                >
                  Ver pedido →
                </button>
          ...
          <div className="flex-1 min-h-0">{kanbanBoard}</div>
        )}
      </div>
      ...
      {showOrder && pageTab === "nueva" && (
        <div className="lg:hidden fixed inset-0 z-50 flex flex-col">
          <div
            ...
            onClick={() => status !== "success" && setShowOrder(false)}
          />
          <div
            className="bg-white rounded-t-2xl shadow-xl flex flex-col"
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 sm:p-0">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm flex flex-col max-h-[90vh]">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  {cobrarOrder.customer?.name ??
                    cobrarOrder.customerName ??
                    "Anónimo"}
                </p>
              </div>
            ...
            <div className="p-4 space-y-3 overflow-y-auto flex-1">
              ...
              {cobrarOrder.discountAmount &&
                Number(cobrarOrder.discountAmount) > 0 &&
                (() => {
                  const disc = Number(cobrarOrder.discountAmount);
                  const original = cobrarTotal + disc;
                  const snap = cobrarOrder.discountsSnapshot;
                  const label =
                    snap?.label ??
                    snap?.name ??
                  ...
                  return (
                    <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2.5 space-y-1">
                      ...
                      <div className="flex justify-between text-sm font-bold text-gray-900 border-t border-emerald-100 pt-1">
                        <span>Total con descuento</span>
                        <span>{fmt(cobrarTotal)}</span>
                      </div>
                    </div>
                  );
                })()}
              {/* Computed promo discount (when cashier selects payment method) */}
              {cobrarComputedDiscount &&
                (() => {
                  const disc = cobrarComputedDiscount.amount;
                  const d = cobrarComputedDiscount.discount;
                  const label =
                    d.label ??
                  ...
                  return (
                    <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2.5 space-y-1">
                      ...
                      <div className="flex justify-between text-sm font-bold text-gray-900 border-t border-emerald-100 pt-1">
                        <span>Total con descuento</span>
                        <span>{fmt(cobrarTotal - disc)}</span>
                      </div>
                    </div>
                  );
                })()}
              ...
              {cobrarPayments.length === 1 &&
                (() => {
                  ...
                  const diff2 = adjTotal - baseAfterDiscount;
                  return (
              ...
              <div
                className={`grid gap-2 ${activePaymentMethods.length <= 3 ? "grid-cols-3" : "grid-cols-2 sm:grid-cols-3"}`}
              >
                {activePaymentMethods.map((m) => {
                  ...
                  return (
                    <button
                      key={m.value}
                      onClick={() => toggleCobrarMethod(m.value)}
                    ...
                    >
                      ...
                      <span className="text-[10px] font-semibold mt-1 leading-tight text-center">
                        {m.label}
                      </span>
                      {m.cfg?.adjustmentType !== "none" && (
                        <span
                      ...
                      {sel && (
                        <input
                          ...
                          onChange={(e) => {
                            e.stopPropagation();
                            updateCobrarAmount(m.value, e.target.value);
                          }}
                          onClick={(e) => e.stopPropagation()}
              ...
              {cobrarPayments.find((p) => p.method === "ONLINE") &&
                orgConfig?.paymentMethods?.mercadopago?.link && (
                  <a
                    href={orgConfig.paymentMethods.mercadopago.link}
                    target="_blank"
              ...
              {cobrarIsCobrado &&
                cobrarPayments.length > 0 &&
                Math.abs(cobrarEffectiveTotal - sumCobrar) > 0.01 && (
                  <p className="text-xs text-rose-600 text-center">
                    {cobrarEffectiveTotal - sumCobrar > 0
                      ? `Faltan ${fmt(cobrarEffectiveTotal - sumCobrar)}`
                      : `Sobran ${fmt(sumCobrar - cobrarEffectiveTotal)}`}
                  </p>
                )}
              {/* Cobrado toggle */}
              <button
                onClick={() => setCobrarIsCobrado(!cobrarIsCobrado)}
              ...
              >
                <span
                  className={`text-sm font-semibold ${cobrarIsCobrado ? "text-emerald-700" : "text-gray-500"}`}
                >
                  {cobrarIsCobrado ? "Cobrado" : "Pendiente de cobro"}
                </span>
                <div
                  className={`w-10 h-6 rounded-full flex items-center px-0.5 transition-colors ${cobrarIsCobrado ? "bg-emerald-500" : "bg-gray-300"}`}
                >
                  <div
                    className={`w-5 h-5 rounded-full bg-white shadow transition-transform ${cobrarIsCobrado ? "translate-x-4" : "translate-x-0"}`}
                  />
                </div>
              </button>
            </div>
            <div className="p-4 border-t border-gray-100 shrink-0">
              <button
                onClick={confirmCobrar}
                disabled={
                  cobrarPayments.length === 0 ||
                  (cobrarIsCobrado &&
                    Math.abs(cobrarEffectiveTotal - sumCobrar) > 0.01) ||
                  cobrarStatus === "loading"
                }
              ...
              >
                {cobrarStatus === "loading"
                  ? "Procesando..."
                  : cobrarIsCobrado
                    ? `Confirmar cobro · ${fmt(cobrarEffectiveTotal)}`
          ...
          <div className="relative bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md flex flex-col max-h-[92vh] sm:max-h-[85vh]">
              ...
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-xs text-gray-400">
                    {relativeTime(detailOrder.date)}
                  </span>
                  <span
                    className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                      detailOrder.orderType === "DELIVERY"
                        ? "bg-amber-100 text-amber-700"
                        : detailOrder.orderType === "TAKEAWAY"
                          ? "bg-blue-100 text-blue-700"
                          : "bg-gray-100 text-gray-600"
                    }`}
                  >
                    {ORDER_TYPE_LABELS[detailOrder.orderType]}
                  </span>
                  {!detailOrder.isPaid &&
                    (() => {
                      const paid = detailOrder.payments.reduce(
                        (s, p) => s + Number(p.amount),
                        0,
                      );
                      ...
                      return paid > 0 ? (
                        ...
                        <span className="text-[10px] bg-rose-100 text-rose-600 rounded-full px-2 py-0.5 font-semibold">
                          Sin cobrar
                        </span>
                      );
                    })()}
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-gray-100">
                ...
                {detailEditItems.map((item) => (
                  ...
                  >
                    <span className="flex-1 text-sm text-gray-800 truncate">
                      {item.name}
                      {item.type === "combo" && (
                        <span className="ml-1 text-[9px] text-violet-400 font-medium">
                          COMBO
                        </span>
                      )}
                    </span>
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        onClick={() =>
                          detailSetQty(item.id, item.type, item.qty - 1)
                        }
                        className="w-6 h-6 rounded border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-50 text-xs font-bold"
                      >
                        −
                      </button>
                      <span className="w-6 text-center text-sm font-semibold tabular-nums">
                      ...
                      {fmt(
                        (item.type === "product"
                          ? (products.find((p) => p.id === item.id)
                              ?.salePrice ?? 0)
                          : (combos.find((c) => c.id === item.id)?.salePrice ??
                            0)) * item.qty,
                      )}
                    </span>
                    <button
                      onClick={() => detailSetQty(item.id, item.type, 0)}
                      className="text-gray-300 hover:text-rose-500 transition-colors shrink-0"
                    >
                      <svg
                ...
                <div className="relative pt-1">
                  ...
                  {detailItemSearch &&
                    (() => {
                      ...
                      const matchProds = products
                        .filter((p) => p.name.toLowerCase().includes(q2))
                        .slice(0, 5);
                      const matchCombos = combos
                        .filter((c) => c.name.toLowerCase().includes(q2))
                        .slice(0, 3);
                      if (matchProds.length === 0 && matchCombos.length === 0)
                        return (
                          <p className="text-xs text-gray-400 text-center py-2">
                            Sin resultados
                          </p>
                        ...
                        <div className="absolute z-10 left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
                          {matchProds.map((p) => (
                            ...
                            >
                              ...
                              <span className="text-emerald-600 font-semibold text-xs shrink-0 ml-2">
                                {fmt(p.salePrice)}
                              </span>
                            </button>
                          ))}
                          {matchCombos.map((c) => (
                            ...
                            >
                              ...
                              <span className="text-violet-600 font-semibold text-xs shrink-0 ml-2">
                                {fmt(c.salePrice)}
                              </span>
                            </button>
                          ))}
              ...
              {detailOrder.orderType === "DELIVERY" && (
                <div className="px-5 py-4 space-y-2">
                  ...
                  <input
                    type="text"
                    placeholder="Dirección de entrega..."
                    value={detailEditDeliveryAddress}
                    onChange={(e) =>
                      setDetailEditDeliveryAddress(e.target.value)
                    }
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                  />
                  {repartidores.length > 0 && (
                    <select
                      value={detailEditRepartidorId}
                      onChange={(e) =>
                        setDetailEditRepartidorId(e.target.value)
                      }
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                    >
              ...
              <div className="px-5 py-4 space-y-3">
                {detailOrder.discountAmount &&
                Number(detailOrder.discountAmount) > 0 ? (
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-sm text-gray-500">
                      <span>Subtotal</span>
                      <span>{fmt(getDetailSubtotal())}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm text-emerald-700">
                      <span>
                        {detailOrder.discountsSnapshot?.label ??
                          detailOrder.discountsSnapshot?.name ??
                          "Descuento"}
                      </span>
                      <span className="font-semibold">
                        −{fmt(Number(detailOrder.discountAmount))}
                      </span>
                    </div>
                ...
                {detailOrder.orderType === "DELIVERY" &&
                  (Number(orgConfig?.deliveryFee) || 0) > 0 && (
                    <p className="text-xs text-amber-600 text-right -mt-2">
                      incl. {fmt(Number(orgConfig?.deliveryFee))} de envío
                    </p>
                  )}
                ...
                >
                  <span
                    className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 ${detailOrder.isPaid ? "bg-emerald-500 border-emerald-500" : "border-amber-400"}`}
                  >
                    {detailOrder.isPaid && (
                      <svg
            ...
            <div className="p-4 border-t border-gray-100 shrink-0 space-y-2">
              ...
              <div className="flex gap-2">
                {STATUS_NEXT[detailOrder.orderStatus] && (
                  ...
                  >
                    {movingId === detailOrder.id
                      ? "..."
                      : STATUS_NEXT_LABEL[detailOrder.orderStatus]}
                  </button>
                )}
                <button
                  onClick={() => {
                    setDetailId(null);
                    setCancellingId(detailOrder.id);
                  }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xs p-5 space-y-4">
            ...
            <p className="text-sm text-gray-600">
              {cancellingOrder.customer?.name ??
                cancellingOrder.customerName ??
                "Anónimo"}{" "}
              — {fmt(Number(cancellingOrder.total))}
            </p>

            {/* Stock decision toggle */}
            <button
              onClick={() =>
                setCancelStockDecision(
                  cancelStockDecision === "deduct" ? "skip" : "deduct",
                )
              }
              className={`w-full flex items-center justify-between gap-3 p-3 rounded-xl border-2 transition-all ${cancelStockDecision === "deduct" ? "border-amber-400 bg-amber-50" : "border-gray-200 bg-gray-50"}`}
            >
              <div className="text-left">
                ...
                <p className="text-xs text-gray-400">
                  Se preparó comida con insumos
                </p>
              </div>
              <div
                className={`w-10 h-6 rounded-full transition-colors flex items-center px-0.5 shrink-0 ${cancelStockDecision === "deduct" ? "bg-amber-500" : "bg-gray-300"}`}
              >
                <div
                  className={`w-5 h-5 rounded-full bg-white shadow transition-transform ${cancelStockDecision === "deduct" ? "translate-x-4" : "translate-x-0"}`}
                />
              </div>
            </button>

            {/* Cash decision toggle — only when order was paid */}
            {cancellingOrder.isPaid && (
              <button
                onClick={() =>
                  setCancelCashDecision(
                    cancelCashDecision === "add" ? "skip" : "add",
                  )
                }
                className={`w-full flex items-center justify-between gap-3 p-3 rounded-xl border-2 transition-all ${cancelCashDecision === "add" ? "border-emerald-400 bg-emerald-50" : "border-gray-200 bg-gray-50"}`}
              >
                <div className="text-left">
                  <p className="text-sm font-medium text-gray-700">
                    Agregar a caja
                  ...
                  <p className="text-xs text-gray-400">
                    El cliente pagó y no devuelve
                  </p>
                </div>
                <div
                  className={`w-10 h-6 rounded-full transition-colors flex items-center px-0.5 shrink-0 ${cancelCashDecision === "add" ? "bg-emerald-500" : "bg-gray-300"}`}
                >
                  <div
                    className={`w-5 h-5 rounded-full bg-white shadow transition-transform ${cancelCashDecision === "add" ? "translate-x-4" : "translate-x-0"}`}
                  />
                </div>
              </button>
            )}
            {cancellingOrder.isPaid && cancelCashDecision !== "add" && (
              <p className="text-xs text-gray-400 -mt-2">
                El cobro será revertido al cancelar.
              </p>
            )}
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2.5 border border-gray-200 text-gray-700 rounded-xl font-medium text-sm hover:bg-gray-50 transition-colors"
              >
                Volver
              </button>
              <button
                onClick={confirmCancel}
                disabled={cancellingStatus === "loading"}
      ...
      {extrasModalProduct && (
        <ExtrasModal
          ...
          extras={getExtrasForProduct(
            {
              id: extrasModalProduct.id,
              categoryId: extrasModalProduct.categoryId,
            },
            allExtras,
          )}
          onConfirm={(sel) => addProductWithExtras(extrasModalProduct, sel)}
          primaryColor="#0f2f26"
